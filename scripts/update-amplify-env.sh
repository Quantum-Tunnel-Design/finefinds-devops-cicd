...
update_amplify_env "$APP_ID_CLIENT" "client.env.json" "$ENVIRONMENT" "$COGNITO_DETAILS" false

# Update admin app environment variables
echo "Updating admin app environment variables..."
update_amplify_env "$APP_ID_ADMIN" "admin.env.json" "$ENVIRONMENT" "$COGNITO_DETAILS" true
